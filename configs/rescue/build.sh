...
make_customize_rootfs() {
    ...
    part_three() {
        ...
        echo "Adding rescue user"
        MKARCHISO_RUN 'useradd -m -g users -G "audio,disk,optical,wheel,network,autologin,nopasswdlogin" rescue'

        # Set rescue account passwordless
        MKARCHISO_RUN 'passwd -d rescue'
    ...
    part_four() {
        ...
        #echo "rescue:U6aMy0wojraho" | chpasswd -R /antergos-iso/configs/antergos/${ROOTFS}
        ...
        if [ -f "${ROOTFS}/usr/share/applications/chromium.desktop" ]; then
            sed -i 's|^Exec=chromium %U|Exec=chromium --user-data-dir=/home/rescue/.config/chromium/Default --start-maximized --homepage=https://antergos.com|g' ${ROOTFS}/usr/share/applications/chromium.desktop
        fi
    ...
    part_five() {
        ...
        MKARCHISO_RUN 'systemctl -fq enable systemd-timesyncd bluetooth'

        # Fix /home permissions
        MKARCHISO_RUN 'chown -R rescue:users /home/rescue'
